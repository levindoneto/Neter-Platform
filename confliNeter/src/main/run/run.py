...
####################################################################################################
		...
		dicAuxFire = {}   # Fica com o primeiro item da lista {0:lista0}

		# Contadores
		contF = 0     # Indice do fluxo
		contSwitchFW = 0
		...
		#string_firewall = "" # String para formar o dicionario a partir do arquivo json

		dicFirewall = {}     # Inicializando dicionarios com as regras do Firewall
		    	             # { SWITCH : [Lista de Fluxos] }
				     # Lista de fluxos: [ {fluxoXY:{Dicionarios de infos do fluxo XY},...} ]
		lista_switchesFirewall = []
		# Criando o arquivo com os dados de fluxos
		...
		for f in fluxos_firewall:
		    string_firewall += f
		'''
		formatarJson("../data/regrasFirewall.json", "../data/arquivo_firewallJson")
		arquivo_Firewall = "regrasFirewall.json"

		with open(arquivo_Firewall) as fw:
		    dataFW = fw.read()
		    json_dataFW = json.loads(dataFW)

		#print "JSON DATA>>>", json_dataFW


		# Variaveis de iteracao:
		#    sFW  -> Switches do Firewal
		#    fsFW -> Fluxos dos switches do Firewall
		lista_switchesFirewall = json_dataFW.keys()           # Montando lista de switches, que armazenam os fluxos, e os fluxos as informacoes das regras
		for sFW in range(len(lista_switchesFirewall)):        # Percorrendo lista de switches
			print "SWICH: ", lista_switchesFirewall[sFW]
			for fsFW in range(len(lista_switchesFirewall[sFW])):
				print "COLOCAR COISAS AQUI"
		...
		json_dataFW = json_dataFW[str(lista_switchesFirewall[0])] # Tem que salvar isso no nome do switch
		#json_dataFW = json_dataFW["flows"]
		print "JSON DATA>>>", json_dataFW

		lista_teste = []
		lista_teste = json_dataFW
		print "lista >>> ",lista_teste[0]          # CADA FLUXO TEM UM DICIONARIO

		# Dicionario de dados recebendo informacoes do arquivo gerado
		#lista_switchesFirewall = json_dataFirewall.keys()

		print "\n", json_dataFW[0]            # Num for, tem que pegar todos dicionarios de regras, aqui so' esta' pegando a primeira regra, do primeiro switch, fazer for pro switch tambemk
		#print "\n\n", type(dicFirewall)


		##----+++++++ FOR FOR ++++++++++++++++++++++++++++++++++++++++
		# Aqui vai ter que ter for pros switches e depois um for dentro para os fluxos


		dicAuxFlow = {}
		dicAuxFlow = json_dataFW[0]           # 0 -> n
		dicAuxFlowKeys = dicAuxFlow.keys()
		#print "\n\n Flows - Keys>>>> ", dicAuxFlowKeys ,"\n\n"
		#print "\n\ndicaux>>>", dicAuxFlow["flow-mod-5"], "\n\n"

		dicAuxInternoFlows = {}
		dicAuxInternoFlows = dicAuxFlow["flow-mod-5"]
		print "\n\nKEYS>>", dicAuxInternoFlows.keys()
		print "\n\nVALUES>>", dicAuxInternoFlows.values(), "\n\n"

		# MATCH DEVE SER CHAMADO ASSIM, ACTIONS TAMBEM:
		# dicAuxInternoFlows = dicAuxFlow["flow-mod-5"]["match"]
